...
static void create_table_desc(TDescriptorTable& t_desc_table, TTableDescriptor& t_table_desc,
                        std::vector<std::string> table_column_names, std::vector<TPrimitiveType::type> types){
        t_table_desc.id = 0;
...
TEST_F(ParquetReaderTest, normal) {
    ...
    std::vector<std::string>  table_column_names = {"boolean_col", "tinyint_col", "smallint_col",
                        ...
                        "double_col"};
    std::vector<TPrimitiveType::type> table_column_types = {TPrimitiveType::BOOLEAN,TPrimitiveType::TINYINT,
                                                      TPrimitiveType::SMALLINT,TPrimitiveType::INT,
                                                      TPrimitiveType::BIGINT,TPrimitiveType::FLOAT,TPrimitiveType::DOUBLE};
    ...
    static_cast<void>(local_fs->open_file(
            "/mnt/disk1/changyuwei/doris/be/test/exec/test_data/parquet_scanner/type-decoder.parquet", &reader));
...
static ParquetReader* create_parquet_reader(TFileScanRangeParams& scan_params,
    ...
    auto local_fs = io::global_local_filesystem();
    io::FileReaderSPtr reader;
    static_cast<void>(local_fs->open_file("/mnt/disk1/changyuwei/doris/be/test/exec/test_data/parquet_scanner/type-decoder.parquet", &reader));
...
TEST_F(ParquetReaderTest, use_column_name) {
    ...
    static_cast<void>(p_reader->init_reader(table_column_names, {}, &colname_to_value_range, {},
            nullptr, nullptr, nullptr, nullptr, nullptr,false, use_column_name));

    std::vector<std::string> read_columns_ans = {"tinyint_col", "smallint_col",
                                             "int_col",     "bigint_col","boolean_col",  "float_col", "double_col"};
...
TEST_F(ParquetReaderTest, use_column_name2) {
    ...
    std::vector<std::string>  table_column_names = {"boolean_col", "tinyint_col", "smallint_col",
                                                    "int_col",     "bigint_col",  "float_col", "test1",
                                                    "double_col",  "test2" };
    std::vector<TPrimitiveType::type> table_column_types = {TPrimitiveType::BOOLEAN,TPrimitiveType::TINYINT,
                                                            TPrimitiveType::SMALLINT,TPrimitiveType::INT,
    ...
    static_cast<void>(p_reader->open());
    static_cast<void>(p_reader->init_reader(table_column_names, {"boolean_col"}, &colname_to_value_range, {},
    nullptr, nullptr, nullptr, nullptr, nullptr,false, use_column_name));
...
TEST_F(ParquetReaderTest, use_column_idx) {
    ...
    std::vector<std::string> read_columns_ans = {"tinyint_col", "smallint_col", "bigint_col",  "string_col"};
    EXPECT_EQ(p_reader->_read_columns, read_columns_ans);

    std::vector<std::string> miss_columns_ans = {"col100","col102"};
    EXPECT_EQ(p_reader->_missing_cols, miss_columns_ans);

    std::vector<std::string> colname_to_value_range_names_ans ={"tinyint_col", "smallint_col", "bigint_col"};
    for (auto col : colname_to_value_range_names_ans) {
        EXPECT_TRUE(p_reader->_colname_to_value_range->contains(col));
    }
    EXPECT_EQ(p_reader->_colname_to_value_range->size(), colname_to_value_range_names_ans.size());
