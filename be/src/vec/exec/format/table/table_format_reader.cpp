...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_parquet_name(
    ...
    for (const auto& slot : table_tuple_descriptor->slots()) {
        ...
        if (file_column_name_idx_map.contains(table_column_name)) {
            ...
            RETURN_IF_ERROR(by_parquet_name(slot->type(), parquet_fields_schema[file_column_idx], field_node));
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_parquet_name(const DataTypePtr& table_data_type , const FieldSchema& file_field,
                              std::shared_ptr<TableSchemaChangeHelper::Node>& node) {
    switch(table_data_type->get_primitive_type()) {
        case TYPE_MAP:{
...

            {
                const auto& key_type = assert_cast<const DataTypePtr&>(
                        assert_cast<const DataTypeMap*>(remove_nullable(table_data_type).get())->get_key_type());

                RETURN_IF_ERROR(by_parquet_name(key_type,
                                                file_field.children[0].children[0],key_node));
            }
            ...
            std::shared_ptr<TableSchemaChangeHelper::Node> value_node = nullptr;
            {
                ...
                RETURN_IF_ERROR(by_parquet_name(value_type,
                                                file_field.children[0].children[1],value_node));
        ...
        case TYPE_STRUCT:{
            ...
            const auto struct_data_type = assert_cast<const DataTypeStruct*>(remove_nullable(table_data_type).get());
            ...
            for (size_t idx = 0; idx < struct_data_type->get_elements().size();idx++)  {
                ...
                if (parquet_field_names.contains(doris_field_name)){
                    ...
                    RETURN_IF_ERROR(by_parquet_name(
                            struct_data_type->get_element(idx), file_field.children[parquet_field_idx], field_node));
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_orc_name(const TupleDescriptor* table_tuple_descriptor,
    ...
    for (const auto& slot : table_tuple_descriptor->slots()) {
        ...
        if (file_column_name_idx_map.contains(table_column_name)) {
            auto file_column_idx =  file_column_name_idx_map[table_column_name];
            std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
            RETURN_IF_ERROR(by_orc_name(slot->type(), orc_type_ptr->getSubtype(file_column_idx), field_node));
            struct_node->add_children(table_column_name,orc_type_ptr->getFieldName(file_column_idx), field_node);
        } else {
            struct_node->add_not_exist_children(table_column_name);
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_orc_name(const DataTypePtr& table_data_type , const orc::Type* orc_root,
                          std::shared_ptr<TableSchemaChangeHelper::Node>& node) {
    switch(table_data_type->get_primitive_type()) {
        case TYPE_MAP: {
        ...
        case TYPE_STRUCT:{
            ...
            for (size_t idx = 0; idx < struct_data_type->get_elements().size() ;idx++)  {
                ...
                if (orc_field_names.contains(doris_field_name)) {
                    ...
                    RETURN_IF_ERROR(by_orc_name(struct_data_type->get_element(idx), orc_root->getSubtype(orc_field_idx) , field_node));
                    struct_node->add_children(doris_field_name, orc_root->getFieldName(orc_field_idx), field_node);
                } else {
                    struct_node->add_not_exist_children(doris_field_name);
                }
            }
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_table_field_id(const schema::external::TField table_schema, const schema::external::TField file_schema,
                                std::shared_ptr<TableSchemaChangeHelper::Node>& node) {
...
//                return Status::NotSupported()
            ...
            RETURN_IF_ERROR(by_table_field_id(*table_schema.nestedField.map_field.value_field,
                                              *file_schema.nestedField.map_field.value_field, value_node));
        ...
        case TPrimitiveType::ARRAY :{
            ...
            RETURN_IF_ERROR(by_table_field_id(*table_schema.nestedField.array_field.item_field,
                                              *file_schema.nestedField.array_field.item_field, item_node));
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_table_field_id(const schema::external::TStructField& table_schema,
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_parquet_field_id(const schema::external::TStructField& table_schema,
                                  const FieldDescriptor& parquet_field_desc,
                                  std::shared_ptr<TableSchemaChangeHelper::Node>& node,
                                  bool& exist_field_id) {
    auto struct_node = std::make_shared<TableSchemaChangeHelper::StructNode>();
    ...
    for (const auto& table_field : table_schema.fields) {
        ...
        if (file_column_id_idx_map.contains(table_field.id)) {
            ...
            std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
            RETURN_IF_ERROR(by_parquet_field_id(table_field, parquet_fields_schema[file_column_idx], field_node, exist_field_id));
            struct_node->add_children(table_column_name,parquet_fields_schema[file_column_idx].name, field_node);
        } else {
            std::cout <<"add not esixts\n";
            struct_node->add_not_exist_children(table_column_name);
        }
    }
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_parquet_field_id(const schema::external::TField& table_schema,
    ...
    switch (table_schema.type.type) {
        case TPrimitiveType::MAP : {
            ...
            RETURN_IF_ERROR(by_parquet_field_id(*table_schema.nestedField.map_field.key_field,
                                                parquet_field.children[0].children[0], key_node, exist_field_id));
        ...
        case TPrimitiveType::STRUCT: {
            ...
            for (const auto& table_field : table_schema.nestedField.struct_field.fields) {
                const auto& table_column_name = table_field.name;
                if (file_column_id_idx_map.contains(table_field.id)) {
                    const auto& file_field = parquet_field.children.at(file_column_id_idx_map[table_field.id]);
                    std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
                    RETURN_IF_ERROR(by_parquet_field_id(table_field, file_field, field_node, exist_field_id));
                    struct_node->add_children(table_column_name, file_field.name, field_node);
                } else {
                    struct_node->add_not_exist_children(table_column_name);
                }
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_orc_field_id(const schema::external::TStructField& table_schema, const orc::Type* orc_root,
    ...
    for (size_t idx =0 ; idx <  orc_root->getSubtypeCount(); idx++) {
        if (orc_root->getSubtype(idx)->hasAttributeKey(field_id_attribute_key)) {
            auto field_id = std::stoi(orc_root->getSubtype(idx)->getAttributeValue(field_id_attribute_key));
    ...
    for (const auto& table_field : table_schema.fields) {
        ...
        if (file_column_id_idx_map.contains(table_field.id)) {
            ...
            std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
            RETURN_IF_ERROR(by_orc_field_id(table_field, file_field, field_id_attribute_key, field_node, exist_field_id));
            struct_node->add_children(table_column_name, orc_root->getFieldName(file_field_idx), field_node);
        } else {
            struct_node->add_not_exist_children(table_column_name);
        }
    }
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_orc_field_id(const schema::external::TField& table_schema, const orc::Type* orc_root,
                              const std::string& field_id_attribute_key,
                              std::shared_ptr<TableSchemaChangeHelper::Node>& node,
                              bool& exist_field_id) {
    switch (table_schema.type.type) {
