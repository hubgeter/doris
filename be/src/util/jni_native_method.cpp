...
Result<int64_t> request_maxcompute_block_id_from_fe(int64_t txn_id,
    ...
    for (uint32_t retry_times = 0; retry_times < FETCH_BLOCK_ID_MAX_RETRY_TIMES; retry_times++) {
        ...
        if (rpc_status.is<ErrorCode::NOT_MASTER>()) {
        ...
        if (result.length != 1) {
            auto msg = "Failed to allocate MaxCompute block_id, expected length=1 but got " +
                       std::to_string(result.length) + " for txn_id=" + std::to_string(txn_id) +
                       ", write_session_id=" + write_session_id;
            LOG(WARNING) << msg;
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            continue;
