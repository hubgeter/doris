...
// size feature can construct a BlockBudget from RuntimeState and use the
// helper methods instead of reimplementing the same row/byte logic inline.
//
// Typical usage:
//   BlockBudget budget(state->block_max_rows(), state->block_max_bytes());
//   size_t eff = budget.effective_max_rows(estimated_row_bytes);
//   while (budget.within_budget(block.rows(), block.bytes())) { ... }
//
struct BlockBudget {
    size_t max_rows;
    size_t max_bytes; // 0 = byte budget disabled

    BlockBudget(size_t max_rows_, size_t max_bytes_) : max_rows(max_rows_), max_bytes(max_bytes_) {}

    bool has_byte_budget() const { return max_bytes > 0; }

    // Pre-compute effective row limit from an estimated average row byte size.
    // When byte budget is disabled or estimated_row_bytes == 0, returns max_rows.
