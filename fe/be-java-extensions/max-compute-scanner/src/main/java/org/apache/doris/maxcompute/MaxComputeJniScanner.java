...
public class MaxComputeJniScanner extends JniScanner {
    ...
    public void open() throws IOException {
        ...
        try {
            ...
            curReader = session.openRecordReader(start, totalRows, true, pushDownColumns);
    ...
    private int readVectors(int expectedRows) throws IOException {
        ...
        while (curReadRows < expectedRows) {
            record = curReader.read();
            if (record == null) {
                break;
            }
            try {
                Column[] fieldColumns = record.getColumns();
                columnValue.reset((ArrayRecord) record);
                for (int recordIdx = 0; recordIdx < fieldColumns.length; recordIdx++) {
                    Column recordColumn = fieldColumns[recordIdx];
                    Integer dstColumnId = readColumnsToId.get(recordColumn.getName());
                    if (dstColumnId == null) {
                        // use for partition if no column need to read.
                        continue;
                    }
                    columnValue.setColumnIdx(recordIdx);
                    appendData(dstColumnId, columnValue);
                }
                if (partitionSpec != null) {
                ...
                throw new RuntimeException("Fail to read data, reason: " + e.getMessage(), e);
            }
        }
        return curReadRows;
    }
