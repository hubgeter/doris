...
public class MaxComputeJniWriter extends JniWriter {
    ...
    public MaxComputeJniWriter(int batchSize, Map<String, String> params) {
        ...
        this.txnId = Long.parseLong(Objects.requireNonNull(params.get(TXN_ID),
                "required property '" + TXN_ID + "'."));
    ...
    protected void writeInternal(VectorTable inputTable) throws IOException {
        ...
        try {
            ...
            writeRowsWithRowChecks(data, numRows, numCols);
        } catch (Exception e) {
    ...
    private void writeRowsWithRowChecks(Object[][] data, int numRows, int numCols) throws IOException {
        int rowStart = 0;
        while (rowStart < numRows) {
            ...
            while (rowEnd < numRows) {
                long rowEstimatedBytes = estimateSingleRowPayloadBytes(data, numCols, rowEnd);
            ...
            if (rowEnd == rowStart) {
                long rowEstimatedBytes = estimateSingleRowPayloadBytes(data, numCols, rowStart);
                batchEstimatedBytes = rowEstimatedBytes;
                rowEnd = rowStart + 1;
                rotateAfterWrite = true;
            }

            try (VectorSchemaRoot root = buildRowRangeRoot(data, numCols, rowStart, rowEnd)) {
    ...
    private long estimateSingleRowPayloadBytes(Object[][] data, int numCols, int rowIndex)
            throws IOException {
        try (VectorSchemaRoot root = buildRowRangeRoot(data, numCols, rowIndex, rowIndex + 1);
    ...
    private VectorSchemaRoot buildRowRangeRoot(Object[][] data, int numCols, int rowStart, int rowEnd) {
        ...
        for (int col = 0; col < numCols && col < columnTypeInfos.size(); col++) {
            fillArrowVector(root, col, columnTypeInfos.get(col).getOdpsType(), data[col], rowStart,
                    rowCount);
        }
        return root;
    }
